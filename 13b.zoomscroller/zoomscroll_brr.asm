#import "../00.music/music.asm"
// possible updates :
// - i think we can remove 1 block from d012table? but we have to make it wrap around for the rasterbar check
// - improve Y zoom code : can we save 2 more blocks?

// - buffers    : can we move the buffers into 1 page?
...
// done : saved 1 block for each shiftTable (the 2nd block was not needed by doing AND #3)
...
.label scroll2Invisible = $19 // scroller 2 too low (FLD'ed off the screen)

.label scroll1Low  = $20
.label scroll1High = $21
.label scroll2Low  = $22
.label scroll2High = $23

.label mapData1          = $2000  //..$3c77
...
{
  .align $100
  * = * "[DATA] conversion tables for shadow up"
  .for (var shadow=0; shadow<9; shadow++)
  {
...
start:
  ...
  lda #($10*((screen1&$3c00)/$400)+$2*(charset0&$3800)/$800)
  sta $d018
  .if (romfont)
  {
    lda #$c8
...
// this functions controls the part
/*
script:
{
...
checkD012:
  ...
  lda d012Tab2+10       // todo : check if +10 is the correct one
  ldx #bottomColor
...
nextChar:
  ...
  {
    lda #0
    sta update.repeat1             // go the next part of movement
    beq nextChar
  }!:
...
continue:
  ...
  */
  ...
  ldy fldIrq.nrLines
  cpx #16
  bcs BRRRoutine16_32  // both routines can handle size 16, but the routine for 16_32 is a bit faster
    jmp BRRRoutine0_16

BRRRoutine16_32:
  // BRR routine for sizes 16-32
  .for (var step=0; step<15; step++)
  {
    .if (step>0)
    {
      cpx #(brrYZoom.get(step)+16+1)
      bcc !+
        iny
      !:
    }
    lda d011Table+(step+1)*4,y
    sta d011Tab1+step+1
    lda d012Table+(step+1)*4,y
    sta d012Tab1+step    // last read is d012Tab1+14
  }
  // note : we have to add the steps to y that were put into reading the table (step*4)
  tya
  clc
  adc #15*4
  tay
...
BRRRoutine0_16:
  .for (var step=0; step<15; step++)
  {
    .if (step>0)
    {
      cpx #(brrYZoom.get(step)+1)
      bcc !+
        iny
      !:
    }
    lda d011Table+(step+1)*3,y
    sta d011Tab1+step+1
    lda d012Table+(step+1)*3,y
    sta d012Tab1+step    // last read is d012Tab1+14
  }
  // note : we have to add the steps to y that we missed
  tya
  clc
  adc #15*3
  tay
...
{
  inc index
  ldy index: #32+10 // out of phase from calcYZoom1
...
.var scroll10 = " B at x2008 all coders became wizards          ADnow -=element*54*=- have become warlocks    dontE forgetF to code, pixel and compose and beat us at x-2028  #hammerfist, #wvl signing off  @"
...
.var scroll20 = " [[[[[[[[[[[[[[ make it look effortless and beat us at x-2028   #genius, #youth signing off  @"
