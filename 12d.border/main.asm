...
.label firstZP       = $10
  ...
  .label d016Logo1   = $12
  .label ypos        = $13
  .label jmpIrq1     = $14 // 14,15,16
  .label jmpResetIrq = $17 // 17,18,19
.label lastZP        = $19
  ...
  *=spindleLoadAddress-18-9-3 "Spindle header"
  .label spindleHeaderStart = *
    ...
    .byte 'I', >sprites,   >spriteEnd  // inherit sprites from previous part
    .byte 'I', >charset,   >(screen1+$3e8) // inherit chars from previous part
...
// calculate x positions
{
  ...
  .var maxYPosition = 15*8
  .var maxXPosition = middle + 48*8
  .var minXPosition = middle - 48*8

  .while ((position < maxXPosition) && (position > minXPosition))
  {
    // update speed
    ...
    .eval xPositions.add(min(round(position), maxXPosition))
    .eval yPositions.add(maxYPosition)
  }
}
